...
async function fetchAssetTransfers(alchemy: Alchemy, address: string, categories: AssetTransfersCategory[]): Promise<AssetTransfersResult> {
    // @ts-ignore
...
async function processTransfers(result: AssetTransfersResult): Promise<Map<string, Interaction>> {
    ...
    for (const transfer of result.transfers) {
        ...
        if (!interactions.has(toAddress)) {
            interactions.set(toAddress, { toContractAddress: toAddress, toContractLabel: null, txnsCount: 0, txnsStats: new Map() });
        }
        ...
        if (!interaction.txnsStats.has(asset)) {
            interaction.txnsStats.set(asset, { asset, assetContractAddress: contractAddress, count: 0, value: 0});
        }
...
function sortAndLimitInteractions(interactions: Map<string, Interaction>, limit: number): Interaction[] {
...
async function populateContractLabels(interactions: Interaction[]) {
    for (const interaction of interactions) {
        interaction.toContractLabel = await getEnsName(interaction.toContractAddress as `0x${string}`);
    }
}

async function getTopInteractions(address: string, networkConfig: NetworkConfig): Promise<Interaction[]> {
    const alchemy = new Alchemy({ apiKey: ALCHEMY_API_KEY, network: networkConfig.network });

    try {
        const result = await fetchAssetTransfers(alchemy, address, networkConfig.categories);
        ...
        console.error(`Error fetching transactions for ${networkConfig.name}:`, error);
        return [];
    }
}

async function getTransactionsForAllNetworks(address: string): Promise<IdentityHubRecord[]> {
